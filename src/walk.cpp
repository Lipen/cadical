...
Walker::Walker(Internal *i, double size, int64_t l) : internal(i),
...
int Internal::walk_round(int64_t limit, bool prev) {
    ...
    if (!failed) {
        ...
        if (minimum < old_global_minimum)
            PHASE("walk", stats.walk.count,
                  "%snew global minimum %" PRId64 "%s in %" PRId64
                  " flips and "
                  "%" PRId64 " propagations",
            ...
            PHASE("walk", stats.walk.count,
                  "best phase minimum %" PRId64 " in %" PRId64
                  " flips and "
                  "%" PRId64 " propagations",
